...
export const simulateAiThrow = (remaining: number, skill: AiProfile, time: number): { result: ThrowResult; region: ScoringRegion } => { const intended = chooseAiTarget(remaining); const target = { x: intended.x + normalSample(0, skill.variance * .35), y: intended.y + normalSample(0, skill.variance * .35) }; const result = finalImpact(target, .5 + normalSample(0, .15), skill, time); return { result, region: scorePoint(result.finalPoint) } }
